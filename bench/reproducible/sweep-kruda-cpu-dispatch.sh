#!/usr/bin/env bash
# Kruda-only CPU-bound dispatch sweep.
#
# This is candidate-discovery tooling, not cross-runtime benchmark evidence.
# It runs CPU-bound routes across Kruda dispatch modes and writes an aggregate
# median summary from the per-run bench.sh summaries.
#
# Usage:
#   ./sweep-kruda-cpu-dispatch.sh

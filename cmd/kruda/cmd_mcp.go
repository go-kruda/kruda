...
kruda.Post[CreateUser, User](app, "/users", func(c *kruda.C[CreateUser]) (*User, error) {
    // c.In is parsed and validated — validation is on by default
...
The validate step runs by default; kruda.WithoutValidation() removes it.
...
// Structured validation errors (from typed handlers)
// Response:
// {
...
read and the upload is unlimited.
...
` + "```" + `go
app := kruda.New(kruda.WithBodyLimit(8<<20)) // else 5mb below is unreachable
...
Kruda implements these rules and no others:

    required email url uuid oneof len

...
**Validation runs by default** since v1.7.1. Opt out with `kruda.New(kruda.WithoutValidation())`, which leaves `validate` tags inert: the request is parsed and nothing is checked. `kruda.WithValidator` is for registering custom rules or messages, not for switching validation on. The modifiers `omitempty` (skip a field's rules when its value is the zero value) and `dive` (apply the rules after it to each element of a slice, array or map) are supported; a `dive` failure names the element, e.g. `ids[2]`. A tag naming a *rule* Kruda does not implement — the rule set is smaller than `go-playground/validator`'s — is skipped with a startup warning rather than panicking; `(*kruda.Validator).RuleNames()` lists what a validator recognises, including rules added with `Register`, but not the modifiers, which are not rules and cannot be registered.

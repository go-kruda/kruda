...
func generate(filename string, src []byte, typeName, funcName string) ([]byte, error) {
	...
	fmt.Fprintf(&body, "func %s(c *Ctx) (reflect.Value, error) {\ninput := new(%s)\n", funcName, typeName)
	...
	for _, source := range []string{"query", "param"} {
		for _, f := range fields {
			...
			if f.kind == "string" {
				...
				fmt.Fprintf(&body, "if err != nil { return reflect.Value{}, BadRequest(%q) }\n", message)
	...
	body.Write(generateValidator(typeName, funcName, fields, validationSupported))
	var out bytes.Buffer
	fmt.Fprintf(&out, "// Code generated by internal/bindgen; DO NOT EDIT.\n\npackage %s\n\nimport (\n\"reflect\"\n", f.Name.Name)
	if useMath {
